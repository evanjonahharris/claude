...
export function TodosView() {
  ...
  const fetchLeads = useCallback(async (p = 1, q = search) => {
    setLoading(true);
    try {
      const params = new URLSearchParams({ filter: 'todos', page: String(p), limit: String(LIMIT), search: q });
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-2">
        {loading ? (
          ...
          <div className="text-center py-12 text-slate-400">
            <p className="text-lg font-medium">No to-dos</p>
            <p className="text-sm mt-1">All leads are up to date or filtered out.</p>
          </div>
        ) : (
          leads.map((lead) => (
            <LeadCard
